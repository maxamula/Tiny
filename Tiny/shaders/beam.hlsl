...
//--------------------------------------------------------------------------------------
// Vertex Shader
//   generates a full-screen triangle via SV_VertexID.
//--------------------------------------------------------------------------------------
...
//   translates your GLSL mainImage into HLSL
//--------------------------------------------------------------------------------------
float4 PSMain11212(VSOutput IN) : SV_TARGET
{
    // reconstruct fragCoord in pixels
    float2 fragCoord = IN.uv * iResolution;

    // --- your original math, unmodified ---
    float2 p = (2.0f * fragCoord - iResolution) / iResolution.y;
    const float tau = 6.28318530718f;
    float a = atan2(p.x, p.y);
    float r = length(p) * 0.75f;
    ///r = r * 1.5 * sin(iTime * 1.5f);
    float2 uv = float2(a / tau, r);
    

    // compute horizontal colour cycling
...
float4 PSMain2(VSOutput IN) : SV_TARGET
{
    float2 fragCoord = IN.uv * iResolution;
    float2 u = fragCoord;
    ...
    for (float i = 0.0; ++i < 19.0;)
    {
        ...
        // build a 2×2 matrix from 4 cosines
        float f = i + 0.02 * t;
...
{
    // reconstruct Shadertoy’s fragCoord
    float2 fragCoord = IN.uv * iResolution;
    float2 I = fragCoord;

    // initialize all your accumulators
    float t = iTime;
    float z = 0.0;
    float d;
    float4 O = float4(0.0, 0.0, 0.0, 0.0);

    // outer “100‐step” marching loop
    [unroll]
    for (int iter = 0; iter < 100; ++iter)
    {
        // build the ray‐march sample point
...
float4 PSMain4(VSOutput IN) : SV_Target
{
    float2 fragCoord = IN.uv * iResolution;
